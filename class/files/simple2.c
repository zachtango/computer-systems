//Store string length before storing the actual string
...
int main() {
	...
	int fd = open("simple2.out", O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
	//write null char at end of string too.
	int n = strlen(name)+1;
	write(fd, &n, sizeof(int)); //write string length first.
	write(fd, name, n);
